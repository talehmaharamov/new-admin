...
@section('title',__('backend.sub-categories'))
@section('content')
        ...
        <div class="page-content">
            <div class="container-fluid">
                <div class="row justify-content-center">
                    <div class="col-xl-9">
                        <div class="card">
                            <form action="{{ route('backend.sub-categories.update',$category->id) }}"
                                ...
                                <div class="card-body">
                                    <div class="col-12">
                                        <div
                                            ...
                                            <h4 class="mb-sm-0">@lang('backend.sub-categories'):
                                                #{{ $category->id }}</h4>
                                        </div>
                                    </div>
                                    ...
                                    <div class="tab-content p-3 text-muted">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control" name="altCategory">
                                                @foreach($categories as $cat)
                                                    <option value="{{ $cat->id }}"
                                                            @if($cat->id == $category->alt_category_id) selected @endif>{{ $cat->translate(app()->getLocale())->name ?? '-' }}
                                                        ( {{ $cat->category()->first()->translate(app()->getLocale())->name ?? '-' }}
                                                        )
                                                    </option>
                                                @endforeach
                                            </select>
                                            {!! validation_response('backend.alt-categories') !!}
                                        </div>
                                    </div>
                                </div>
